...
URL_VIDA_PROGRAMADOR_TAG_TIRINHA = (
    BASE_URL_VIDA_PROGRAMADOR + "tag-tirinhas.html"
)
URL_VIDA_PROGRAMADOR_TAG_REAL = BASE_URL_VIDA_PROGRAMADOR + "tag-real.html"
URL_VIDA_PROGRAMADOR_TAG_USUARIO = (
    BASE_URL_VIDA_PROGRAMADOR + "tag-usuario.html"
)
...
def get_meme_url_image():
    ...
    while not meme_found:
        ...
        for url in URLS_VERIFICAR_VIDA_PROGRAMADOR:
            tags_found = bool(
                document.select('.postmetadata > a[href="{}"]'.format(url))
            )
            if tags_found:
                break
