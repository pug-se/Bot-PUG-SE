...
class Cache(peewee.Model):
    ...
    def set_value(cls, key, result, expire):
        ...
        try:
            ...
            cached_item = cls.create(
                key=key, result=result, expire_time=expire_time,
            )
        return cached_item
    ...
    def get_value(cls, key):
        ...
        try:
            cached_result = cls.get((cls.key == key) & (cls.expire_time > now))
            cache_logger.info(
                f"Hit for key: {key}"
    ...
    def remove_value(cls, key):
        ...
        try:
            cached_value = cls.get(cls.key == key)
...
class CommandInfo(peewee.Model):
    ...
    class Meta:
        ...
        database = _db
        primary_key = peewee.CompositeKey("command_name", "key")
    ...
    def set_value(cls, command_name, key, info):
        ...
        try:
            command_info = cls.get(
                (cls.command_name == command_name) & (cls.key == key)
            )
            command_info.info = info
            command_info.save()
        except:
            command_info = cls.create(
                command_name=command_name, key=key, info=info,
            )
        return command_info
